...
function formatEtherAddressToShort(address) {
  if (!address || address === NATIVE_ADDRESS) return "...";
  const firstPart = address.slice(0, 5);
  const secondPart = address.slice(-4);
  return `${firstPart}...${secondPart}`;
}
function removeTrailingZeroesFromNumberString(str) {
  return String(Number(str));
}
function formatEtherQuantityShort(quantity) {
  const precision = 4;
...
async function getUserBalance(token) {
  ...
  if (token.address[chainId] === NATIVE_ADDRESS) {
    ...
    userBalance = balance ? Number(balance) / 10 ** token.decimals : 0;
    console.log(userBalance);
    userOtherBalance = anotherBalance_
      ? Number(anotherBalance_) / 10 ** token.decimals
      : 0;
  }
  anotherBalance.innerHTML =
    "Balance: " + userOtherBalance > 0
      ? userOtherBalance.toFixed(token.decimals > 6? 4 : 2)
      : 0 + " " + token.symbol;
  balanceValue.innerHTML = `Balance: ${userBalance?.toFixed(token.decimals > 6? 4 : 2)} ${
    token.symbol
  }`;
}
  ...
  if ( !account) {
    return;
      ...
      if (token.address[chainId] === NATIVE_ADDRESS) {
       ...
       return  Number(nativeBalance) / 10 ** 18;
...
async function getUsdtRate() {
  ...
  if (ONE_INCH_PRICE_AGRIGATOR_ADDRESSES[chainId]) {
    ...
    usdtRate =
      Number(usdValue.toString()) / 10 ** (6 + (18 - selectedTokenA.balance));
  }
}
    ...
    <div id=${
      ...
      <div id=${
        ...
        <div id=${
          ...
          <a class="crypto-a textDecoration" href="${
            ...
            address[chainId] ? formatEtherAddressToShort(address[chainId]) : "..."
  }</a>
          <span id=${
            address
              ? address[chainId]
      ...
      <span id=${
        address
          ? address[chainId]
          : "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
      } class="span-cost">${balance > 0? balance.toFixed(decimals>6 ? 4 : 2) : '0'} ${symbol}</span>
    </div>
  `;
  newItem.id = address
    ? address[chainId]
    ...
    TOKENS.map(async (token) => {
      ...
      try {
        ...
        return token;
          ...
          balanceFetchCount = balanceFetchCount + 1
